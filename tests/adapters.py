...
import regex as re
def pre_tokenization(s: str, special_token: list) ->list:
    #gpt-2这个正则进行分词
    PAT = r"(?:[sdmt]|ll|ve|re)| ?\p{L}+| ?\p{N}+| ?[^\s\p{L}\p{N}]+|\s+(?!\S)|\s+"
    if not special_token:
        return [s]
    #将special token 单独提出来，不要被正则所匹配了
    union_special_token = '|'.join(re.escape(t) for t in special_token)
    pre_parts = re.split(f'({union_special_token})', s)
    parts = [p for p in pre_parts if p != '']
    set_special = set(special_token)
    output = []
    for part in parts:
        if part in set_special:
            output.append(part)
        else:
            output.extend(re.findall(PAT, part))
    return output
